                ...
                if (status === google.maps.GeocoderStatus.OK && results && results[0] && results[0].address_components) {
                    $.each(results[0].address_components, function (index, value) {
                        if ($.inArray('route', value.types) >= 0) {
                            $(form).find('#street').val(value.long_name);
                        }
                        if ($.inArray('country', value.types) >= 0) {
                            $(form).find('#country').val(value.long_name);
                        }
                        if ($.inArray('country', value.types) >= 0) {
                            $(form).find('#country-code').val(value.short_name);
                            $(form).find('#country').val(value.long_name);
                        }
                        if ($.inArray('administrative_area_level_1', value.types) >= 0) {
                            $(form).find('#state').val(value.short_name);
                        }
                        if ($.inArray('administrative_area_level_2', value.types) >= 0) {
                            $(form).find('#city').val(value.long_name);
                        }
                        if ($.inArray('sublocality_level_1', value.types) >= 0) {
                            $(form).find('#neighborhood').val(value.long_name);
                        }
                    });
                } else {
